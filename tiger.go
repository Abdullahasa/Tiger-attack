...
/*
 TIGER DoS tool on <strike>steroids</strike> goroutines. Just ported from Python with some improvements.
 Original Python utility by Barry Shteiman http://www.sectorix.com/2012/05/17/TIGER-web-server-dos-tool/
...
func main() {
	...
	go func() {
		fmt.Println("-- TIGER Attack Started --\n           Go!\n\n")
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- TIGER Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
